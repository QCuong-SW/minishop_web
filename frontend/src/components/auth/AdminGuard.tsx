...
export function AdminGuard({ children }: { children: React.ReactNode }) {
  const { user, isAdmin, isAuthenticated, isAuthReady } = useAuth();

  // Không dùng setTimeout giả để đoán auth đã load xong.
  // Chờ đúng trạng thái restore session từ AuthContext.
  if (!isAuthReady) {
    return (
        ...
        <div className="max-w-md w-full bg-slate-900 border border-rose-500/30 rounded-3xl p-8 shadow-2xl text-center space-y-6">
          ...
          <div className="bg-slate-800/80 p-4 rounded-2xl border border-slate-700 text-left text-xs space-y-2">
            ...
            <p className="text-slate-400 text-[11px]">
              ...
              <strong>Quản Văn Lý (admin@minishop.vn)</strong> để tiếp tục.
            </p>
          </div>
