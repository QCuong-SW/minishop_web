...
export function Navbar() {
  ...
  return (
    <header className="fixed md:sticky top-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-sm transition-all">
      ...
      {/* Main Navbar */}
      {/* Main Navbar */}
      <div className="w-full h-16 px-3 sm:px-4 lg:px-6 flex items-center gap-3">

        {/* Mobile Menu Button */}
        <button
          type="button"
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          className="p-2 rounded-xl text-slate-600 hover:bg-slate-100 md:hidden flex-shrink-0"
        >
          {isMobileMenuOpen ? (
            <X className="w-6 h-6" />
          ) : (
        ...
        {/* Right Navigation */}
        <nav className="flex items-center gap-2 md:gap-3 lg:gap-4 text-sm font-medium text-slate-700 flex-shrink-0">

          <Link
            href="/products"
            className="hidden lg:flex hover:text-shopee-orange transition font-semibold"
          >
            Sản Phẩm
          </Link>
          ...
          <Link
            href="/appointments"
            className="hidden xl:flex items-center gap-1.5 hover:text-shopee-orange transition text-xs font-semibold py-1.5 px-3 rounded-full bg-slate-100 hover:bg-orange-50"
          >
          ...
          >
            ...
            {itemCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-shopee-orange text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center shadow-sm">
                {itemCount}
              </span>
            )}
          </Link>
          ...
          {isCustomerUser ? (
            <div className="relative flex-shrink-0">
              ...
              >
                ...
                <span className="text-xs font-bold text-slate-800 hidden lg:block max-w-[100px] truncate">
                  {user.name}
                </span>
            ...
            <Link
              href="/login"
              className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full bg-shopee-orange text-white font-bold text-xs hover:bg-shopee-hover shadow-md"
            >
